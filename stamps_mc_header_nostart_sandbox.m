function stamps_mc_header_nostart_sandbox(start_step,end_step,patches_flag,est_gamma_parm,patch_list_file)
...
if isdeployed
     start_step = str2num(start_step);
     end_step = str2num(end_step);
     est_gamma_parm = str2num(est_gamma_parm);
end
...
if ~isempty(ix_split_patches)
    ...
    %full parallelizaton for hadoop
    n_cores = n_patches;
    ...
    for k=1:n_cores
        ...
        for ll=1:length(ix)
            ...
            if ~isempty(str_temp)
                %cpcommand = ['cp parms.mat ' str_temp filesep];
                %fprintf(cpcommand);fprintf('\n');
                %system(cpcommand);
                %copyfile('parms.mat',[str_temp filesep 'parms.mat']);
                % copy parms.mat to PATCH
                fp=fopen('parms.mat','r');
                parms=fread(fp);
                fclose(fp);
                fpw=fopen([str_temp filesep 'parms.mat'],'w');
                fwrite(fpw,parms);
                fclose(fpw);
                fprintf(fid,'%s\n',str_temp);
            end
    ...
    fprintf('Patch list split for %d cores\n',n_cores);
    fprintf('patches_flag_str: %s, est_gamma_parm_str: %s\n',patches_flag_str,est_gamma_parm_str);
    comandstr = 'echo logfile > log_stamps_overview';
    system(comandstr);
    
    % nohup needed to run matlab on Linux-Unix
    % matlab looses STAMPS path for some reason when running this way so you need to add it again

    stamps_mat_dir=fullfile(getenv('STAMPS'),'matlab')

    for k=1:n_cores
        comandstr = (['nohup matlab -nodesktop -nodisplay -r "addpath(''' stamps_mat_dir '''); stamps(' num2str(step_range(ix_split_patches(1)) ) ',' num2str(step_range(ix_split_patches(end))) ',' patches_flag_str ',' est_gamma_parm_str ',' '''patch_list_split_' num2str(k) ''',1  ); exit" > log_stamps_split_' num2str(k) ' & ' ]);
